...
namespace bankersheart.Poweradmin.CSR
{
    public partial class View_Yash_Public_Charitable_Trust : System.Web.UI.Page
    {
        ...
        protected void rpt_data_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            {
                Response.Redirect("Manage_Yash_Public_Charitable_Trust.aspx?id=" + hdnID.Value);
            }

            if (e.CommandName == "delete")
            {
                string sQuery = @"delete from [bankers_usr].[YashPublicCharitableTrust] where id='" + hdnID.Value + "'";
                new DAL().ExecuteNonQuery(sQuery, CommandType.Text, null);

                ltr_Success.Text = "Yash_Public_Charitable_Trust deleted.";
                div_Success.Visible = true;
