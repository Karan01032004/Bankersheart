...
namespace bankersheart.Poweradmin.Media.Lectures
{
    public partial class Manage_Lectures : System.Web.UI.Page
    {
        string TableName = "[bankers_usr].[Lectures]";
        ...
        private void fill_data(string id)
        {
            ...
            DataTable dt = new DAL().GetDataTable("SELECT * FROM " + TableName + " WHERE id='" + id + "' ORDER BY Lectures_Date ASC", CommandType.Text, null);
            if (dt.Rows.Count > 0)
            {
            ...
            DataTable dt1 = new DAL().GetDataTable("SELECT image FROM [bankers_usr].[Lectures_Images] WHERE Lectures_id = '" + id + "' ORDER BY added_date DESC", CommandType.Text, null);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                txt_CampDate.Text = Convert.ToDateTime(dt.Rows[0]["Lectures_Date"]).ToString("dd-MM-yyyy");
            }
        ...
        protected void RemoveImage(object sender, CommandEventArgs e)
        {
            ...
            string deleteImageQuery = @"DELETE FROM [bankers_usr].[Lectures_Images] WHERE image = @Image AND Lectures_id = @NewsatBankersId";
        ...
        new SqlParameter("@NewsatBankersId", SqlDbType.Int) { Value = Request.QueryString["ID"].ToString() }
            ...
            string filePath = Server.MapPath("~/poweradmin/WebFiles/Lectures/" + imageToRemove);
            ...
            Response.Redirect("Manage_Lectures.aspx?ID=" + Request.QueryString["ID"].ToString() + "&msg=" + Server.UrlEncode(msg));
        }
        protected void btn_submit_Click(object sender, EventArgs e)
        {
            ...
            foreach (HttpPostedFile uploadedFile in fpbannerimage.PostedFiles)
            {
                ...
                string filepath = "~/poweradmin/WebFiles/Lectures/" + fileName;
            ...
            //    string categoryFilePath = "~/poweradmin/WebFiles/LecturesCategory/" + categoryFileName;
            ...
            //    string categoryFilePathOriginal = "~/poweradmin/WebFiles/LecturesCategory/" + categoryFileName + categoryFileExtension;
            //    string categoryFilePathWebP = "~/poweradmin/WebFiles/LecturesCategory/" + categoryFileName + ".webp";
            ...
            if (Category.HasFile)
            {
                ...
                using (var image = new MagickImage(fullPathOriginal))
                {
                    ...
                    image.Quality = 80; // Standard 80 for optimal web size (100 if you need lossless)
            ...
            string query = @"INSERT INTO " + TableName + " (Browsertitle, MetaTag,title,titlelink, shortdescription, description, Lectures_Date, added_date, added_ip, image)   VALUES(@Browsertitle, @MetaTag,@Title,@titlelink, @ShortDescription, @Description, @EventsatBankersDate, @AddedDate, @AddedIP, @Image); SELECT SCOPE_IDENTITY(); ";
    ...
    new SqlParameter("@MetaTag", SqlDbType.NVarChar) { Value = txtMeta.Text.Replace("'", "''") },
        ...
        new SqlParameter("@Image", SqlDbType.NVarChar) { Value = categoryImageFileName } // Save category image filename
            ...
            {
                string imageQuery = @"INSERT INTO [bankers_usr].[Lectures_Images] (Lectures_id, image, added_date) 
            ...
            string msg = "Lectures added successfully.";
            Response.Redirect("View_Lectures.aspx?msg=" + msg);
        }
        ...
        protected void btn_Update_Click(object sender, EventArgs e)
        {
            ...
            //    // Assign filename for DB update (only store original filename, WebP is inferred)
            //    categoryImageFileName = categoryFileName + categoryFileExtension;
            //}
            if (Category.HasFile)
            {
                ...
                {
                    image.Format = MagickFormat.WebP;
                    image.Quality = 80; // 80-85 optimal web performance ke liye (100 lossless ke liye)
            ...
            List<SqlParameter> parameters = new List<SqlParameter>
    {
        ...
        new SqlParameter("@ID", SqlDbType.Int) { Value = id }
            ...
            foreach (string fileName in uploadedFileNames)
            {
                string imageQuery = @"INSERT INTO [bankers_usr].[Lectures_Images] (Lectures_id, image, added_date) VALUES (@MedicalCampId, @Image, @AddedDate)";
                SqlParameter[] imageParameters = {
            ...
            string msg = "Lectures updated successfully.";
            Response.Redirect("view_Lectures.aspx?msg=" + msg);
        }
        protected void btn_cancel_Click(object sender, EventArgs e)
        {
            Response.Redirect("view_Lectures.aspx");
        }
    }
}
