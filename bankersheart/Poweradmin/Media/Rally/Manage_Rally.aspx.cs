...
namespace bankersheart.Poweradmin.Media.Rally
{
    public partial class Manage_Rally : System.Web.UI.Page
    {
        string TableName = "[bankers_usr].[Rally]";
        ...
        private void fill_data(string id)
        {
            ...
            DataTable dt = new DAL().GetDataTable("SELECT * FROM " + TableName + " WHERE id='" + id + "' ORDER BY Rally_Date ASC", CommandType.Text, null);
            if (dt.Rows.Count > 0)
            {
            ...
            DataTable dt1 = new DAL().GetDataTable("SELECT image FROM [bankers_usr].[Rally_Images] WHERE Rally_id = '" + id + "' ORDER BY added_date DESC", CommandType.Text, null);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                txt_CampDate.Text = Convert.ToDateTime(dt.Rows[0]["Rally_Date"]).ToString("dd-MM-yyyy");
            }
        ...
        protected void RemoveImage(object sender, CommandEventArgs e)
        {
            ...
            string deleteImageQuery = @"DELETE FROM [bankers_usr].[Rally_Images] WHERE image = @Image AND Rally_id = @NewsatBankersId";
        ...
        new SqlParameter("@NewsatBankersId", SqlDbType.Int) { Value = Request.QueryString["ID"].ToString() }
            ...
            string filePath = Server.MapPath("~/poweradmin/WebFiles/Rally/" + imageToRemove);
            ...
            Response.Redirect("Manage_Rally.aspx?ID=" + Request.QueryString["ID"].ToString() + "&msg=" + Server.UrlEncode(msg));
        }
        protected void btn_submit_Click(object sender, EventArgs e)
        {
            ...
            foreach (HttpPostedFile uploadedFile in fpbannerimage.PostedFiles)
            {
                ...
                string filepath = "~/poweradmin/WebFiles/Rally/" + fileName;
            ...
            //    string categoryFilePath = "~/poweradmin/WebFiles/RallyCategory/" + categoryFileName;
            ...
            //    string categoryFilePathOriginal = "~/poweradmin/WebFiles/RallyCategory/" + categoryFileName + categoryFileExtension;
            //    string categoryFilePathWebP = "~/poweradmin/WebFiles/RallyCategory/" + categoryFileName + ".webp";
            ...
            string query = @"INSERT INTO " + TableName + " (Browsertitle, MetaTag,title,titlelink, shortdescription, description, Rally_Date, added_date, added_ip, image)   VALUES(@Browsertitle, @MetaTag,@Title,@titlelink, @ShortDescription, @Description, @EventsatBankersDate, @AddedDate, @AddedIP, @Image); SELECT SCOPE_IDENTITY(); ";
    ...
    new SqlParameter("@MetaTag", SqlDbType.NVarChar) { Value = txtMeta.Text.Replace("'", "''") },
        ...
        new SqlParameter("@Image", SqlDbType.NVarChar) { Value = categoryImageFileName } // Save category image filename
            ...
            {
                string imageQuery = @"INSERT INTO [bankers_usr].[Rally_Images] (Rally_id, image, added_date) 
            ...
            string msg = "Rally added successfully.";
            Response.Redirect("View_Rally.aspx?msg=" + msg);
        }
        ...
        protected void btn_Update_Click(object sender, EventArgs e)
        {
            ...
            if (Category.HasFile)
            {
                ...
                {
                    image.Format = MagickFormat.WebP;
                    image.Quality = 80; // Standard 80 web ke liye (100 agar lossless chahiye)
                    image.Strip();      // Metadata/EXIF strip karne ke liye
            ...
            List<SqlParameter> parameters = new List<SqlParameter>
    {
        ...
        new SqlParameter("@ID", SqlDbType.Int) { Value = id }
            ...
            foreach (string fileName in uploadedFileNames)
            {
                string imageQuery = @"INSERT INTO [bankers_usr].[Rally_Images] (Rally_id, image, added_date) VALUES (@MedicalCampId, @Image, @AddedDate)";
                SqlParameter[] imageParameters = {
            ...
            string msg = "Rally updated successfully.";
            Response.Redirect("view_Rally.aspx?msg=" + msg);
        }
        protected void btn_cancel_Click(object sender, EventArgs e)
        {
            Response.Redirect("view_Rally.aspx");
        }
    }
}
