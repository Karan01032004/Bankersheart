...
namespace bankersheart.Poweradmin.Media.Conference
{
    public partial class Manage_Conference : System.Web.UI.Page
    {
        string TableName = "[bankers_usr].[Conference]";
        ...
        private void fill_data(string id)
        {
            ...
            DataTable dt = new DAL().GetDataTable("SELECT * FROM " + TableName + " WHERE id='" + id + "' ORDER BY Conference_Date ASC", CommandType.Text, null);
            if (dt.Rows.Count > 0)
            {
            ...
            DataTable dt1 = new DAL().GetDataTable("SELECT image FROM [bankers_usr].[Conference_Images] WHERE Conference_id = '" + id + "' ORDER BY added_date DESC", CommandType.Text, null);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                txt_CampDate.Text = Convert.ToDateTime(dt.Rows[0]["Conference_Date"]).ToString("dd-MM-yyyy");
            }
        ...
        protected void RemoveImage(object sender, CommandEventArgs e)
        {
            ...
            string deleteImageQuery = @"DELETE FROM [bankers_usr].[Conference_Images] WHERE image = @Image AND Conference_id = @NewsatBankersId";
        ...
        new SqlParameter("@NewsatBankersId", SqlDbType.Int) { Value = Request.QueryString["ID"].ToString() }
            ...
            string filePath = Server.MapPath("~/poweradmin/WebFiles/Conference/" + imageToRemove);
            ...
            Response.Redirect("Manage_Conference.aspx?ID=" + Request.QueryString["ID"].ToString() + "&msg=" + Server.UrlEncode(msg));
        }
        protected void btn_submit_Click(object sender, EventArgs e)
        {
            ...
            foreach (HttpPostedFile uploadedFile in fpbannerimage.PostedFiles)
            {
                ...
                string filepath = "~/poweradmin/WebFiles/Conference/" + fileName;
            ...
            //    string categoryFilePath = "~/poweradmin/WebFiles/ConferenceCategory/" + categoryFileName;
            ...
            //    string categoryFilePathOriginal = "~/poweradmin/WebFiles/ConferenceCategory/" + categoryFileName + categoryFileExtension;
            //    string categoryFilePathWebP = "~/poweradmin/WebFiles/ConferenceCategory/" + categoryFileName + ".webp";
            ...
            if (Category.HasFile)
            {
                ...
                using (var image = new MagickImage(fullPathOriginal))
                {
                    ...
                    image.Quality = 80; // 80-85 web ke liye optimal hai; 100 agar exact lossless chahiye
            ...
            string query = @"INSERT INTO " + TableName + " (Browsertitle, MetaTag,title,titlelink, shortdescription, description, Conference_Date, added_date, added_ip, image)   VALUES(@Browsertitle, @MetaTag,@Title,@titlelink, @ShortDescription, @Description, @EventsatBankersDate, @AddedDate, @AddedIP, @Image); SELECT SCOPE_IDENTITY(); ";
    ...
    new SqlParameter("@MetaTag", SqlDbType.NVarChar) { Value = txtMeta.Text.Replace("'", "''") },
        ...
        new SqlParameter("@Image", SqlDbType.NVarChar) { Value = categoryImageFileName } // Save category image filename
            ...
            {
                string imageQuery = @"INSERT INTO [bankers_usr].[Conference_Images] (Conference_id, image, added_date) 
            ...
            string msg = "Conference added successfully.";
            Response.Redirect("View_Conference.aspx?msg=" + msg);
        }
        ...
        protected void btn_Update_Click(object sender, EventArgs e)
        {
            ...
            if (Category.HasFile)
            {
                ...
                {
                    image.Format = MagickFormat.WebP;
                    image.Quality = 80; // Standard 80 rakhein (100 agar exact lossless chahiye)
            ...
            List<SqlParameter> parameters = new List<SqlParameter>
    {
        ...
        new SqlParameter("@ID", SqlDbType.Int) { Value = id }
            ...
            foreach (string fileName in uploadedFileNames)
            {
                string imageQuery = @"INSERT INTO [bankers_usr].[Conference_Images] (Conference_id, image, added_date) VALUES (@MedicalCampId, @Image, @AddedDate)";
                SqlParameter[] imageParameters = {
            ...
            string msg = "Conference updated successfully.";
            Response.Redirect("view_Conference.aspx?msg=" + msg);
        }
