...
{
    public partial class insurance_partners : System.Web.UI.Page
    {
        ...
        private void BindAwards(int pageIndex)
        {
            ...
            string query = $@"
    WITH insurance_partners_CTE AS
    (
        SELECT [id], [title], [image], 
               ROW_NUMBER() OVER (ORDER BY id DESC) AS RowNum
        FROM [bankers_usr].[insurance_partners]
    )
    SELECT * FROM insurance_partners_CTE 
    WHERE RowNum BETWEEN {startRow + 1} AND {startRow + pageSize}";
            ...
            string countQuery = "SELECT COUNT(*) FROM [bankers_usr].[insurance_partners]";
