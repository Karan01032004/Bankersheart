using bankersheart.App_Code;
using System;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using System.Text;
using System.Web;
using System.Web.UI;
using System.Web.UI.WebControls;

namespace bankersheart
{
    public partial class achievements_awards : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                int pageIndex = 1;
                if (!string.IsNullOrEmpty(Request.QueryString["awardspage"]))
                {
                    int.TryParse(Request.QueryString["awardspage"], out pageIndex);

                    // ✅ Redirect page=1 to clean URL
                    if (pageIndex == 1)
                    {
                        Response.Status = "301 Moved Permanently";
                        Response.AddHeader("Location", "/achievements-awards");
                        Response.End();
                        return;
                    }
                }

                BindAwards(pageIndex);

                // ✅ Title + H1 handling
                if (pageIndex > 1)
                {
                    Page.Title = "Achievements and Awards - Page " + pageIndex + " | Bankers Group of Hospitals";
                    pageH1.InnerText = "Achievement and Awards - Page " + pageIndex;

                    metaDescription.Attributes["content"] =
                   "Explore award-winning cardiac care and medical excellence at Bankers Heart Institute, Surat. Page " + pageIndex;
                }
                else
                {
                    Page.Title = "Achievements and Awards | Bankers Group of Hospitals";
                    pageH1.InnerText = "Achievements and Award";

                    metaDescription.Attributes["content"] =
                        "Award-winning cardiac care and medical excellence at Bankers Heart Institute, Surat.";
                }

                // ✅ Canonical URL
                string canonicalUrl = "https://www.bankersheart.com/achievements-awards";
                if (pageIndex > 1)
                {
                    canonicalUrl += $"?awardspage={pageIndex}";
                }

                canonicalLink.Attributes["href"] = canonicalUrl;
            }
        }


        private void BindAwards(int pageIndex)
        {
            int pageSize = 12; // Display only 3 records per page
            int startRow = (pageIndex - 1) * pageSize;

            string query = $@"
    WITH Awards_Achivement_CTE AS
    (
        SELECT [id], [visible], [title], [banner_image], 
               ROW_NUMBER() OVER (ORDER BY sortorder DESC) AS RowNum
        FROM [bankers_usr].[Awards_Achivement]   WHERE [visible] = 1
    )
    SELECT * FROM Awards_Achivement_CTE 
    WHERE RowNum BETWEEN {startRow + 1} AND {startRow + pageSize}";

            DataTable dt = new DAL().GetDataTable(query, CommandType.Text, null);
            rptAwards.DataSource = dt;
            rptAwards.DataBind();

            // Fetch total record count
            string countQuery = "SELECT COUNT(*) FROM [bankers_usr].[Awards_Achivement]";
            int totalRecords = Convert.ToInt32(new DAL().ExecuteScalar(countQuery, CommandType.Text, null));
            int totalPages = (int)Math.Ceiling((double)totalRecords / pageSize);

            // Generate pagination controls
            GenerateAwardsPaginationControls(pageIndex, totalPages);
        }
        private void GenerateAwardsPaginationControls(int currentPage, int totalPages)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("<div class='col-12 carousal-arrow'>");

            // Previous Button
            if (currentPage > 1)
            {
                sb.Append($"<a href='?awardspage={currentPage - 1}'><i class='fa fa-long-arrow-left'></i></a>");
            }
            else
            {
                sb.Append("<a href='javascript:void(0)'><i class='fa fa-long-arrow-left text-muted'></i></a>");
            }

            // Page Numbers (1, 2, 3, etc.)
            for (int i = 1; i <= totalPages; i++)
            {
                if (i == currentPage)
                    sb.Append($"<a href='javascript:void(0)' class='px-3 active'>{i}</a>");
                else
                    sb.Append($"<a href='?awardspage={i}' class='px-3'>{i}</a>");
            }

            // Next Button
            if (currentPage < totalPages)
            {
                sb.Append($"<a href='?awardspage={currentPage + 1}'><i class='fa fa-long-arrow-right'></i></a>");
            }
            else
            {
                sb.Append("<a href='javascript:void(0)'><i class='fa fa-long-arrow-right text-muted'></i></a>");
            }

            sb.Append("</div>");

            PaginationAwardsPlaceholder.Controls.Clear();
            PaginationAwardsPlaceholder.Controls.Add(new Literal { Text = sb.ToString() });
        }        //private void BindAwards()
        //{
        //    string query = @"SELECT id, visible, title, banner_image 
        //             FROM bankers_db.bankers_usr.Awards_Achivement
        //             WHERE visible = 1
        //             ORDER BY sortorder ASC";

        //    DataTable dt = new DAL().GetDataTable(query,CommandType.Text,null);
        //    if (dt.Rows.Count > 0)
        //    {
        //        rptAwards.DataSource = dt;
        //        rptAwards.DataBind();
        //    }
        //}
    }
}